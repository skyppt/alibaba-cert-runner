...
def chrome_process_product(url: str, publish: bool, timeout: int) -> dict:
    ...
    try {{
      ...
      if (!finalResult && {str(publish).lower()}) {{
        ...
        sleep(1000);
        const logisticsSelected = waitFor(`() => {{
          ...
          const logisticsSection = [...document.querySelectorAll('.sell-card, div, section')]
            .filter(el => (el.innerText || '').includes('物流提供方式') && (el.innerText || '').includes('智能运费模板'))
            .sort((a, b) => (a.innerText || '').length - (b.innerText || '').length)[0];
          if (logisticsSection) logisticsSection.scrollIntoView({{ block: 'center' }});
          const cards = [...document.querySelectorAll('.LogisticsDeliveryMethod-container-content-list-office, [class*="LogisticsDeliveryMethod"][class*="office"], [class*="delivery"][class*="office"], [class*="DeliveryMethod"]')]
            .filter(el => (el.innerText || '').includes('智能运费模板'));
          const card = cards[0] || [...document.querySelectorAll('div, label')]
            .filter(el => (el.innerText || '').includes('智能运费模板') && (el.innerText || '').includes('可计算核心国家'))
            .sort((a, b) => (a.innerText || '').length - (b.innerText || '').length)[0];
          if (!card) return {{ ok: false, message: 'smart freight template card not found' }};
          card.scrollIntoView({{ block: 'center', inline: 'center' }});
          const clickable = card.querySelector('.LogisticsDeliveryMethod-select, input[type="radio"], input[type="checkbox"], .next-radio, .next-checkbox, .LogisticsDeliveryMethod-container-content-list-office-title') || card;
          const r = clickable.getBoundingClientRect();
          const clickX = Math.max(8, r.left + Math.min(Math.max(r.width / 2, 18), 120));
          const clickY = r.top + r.height / 2;
          const target = document.elementFromPoint(clickX, clickY) || clickable;
          ...
          if (clickable !== target && typeof clickable.click === 'function') clickable.click();
        ...
        const submitState = waitFor(`() => {{
          const text = document.body ? document.body.innerText : '';
          const href = location.href;
          const expected = {json.dumps(expected_item_id)};
          const buttons = [...document.querySelectorAll('button')].filter(el => (el.offsetWidth || el.offsetHeight || el.getClientRects().length));
          const hasContinue = buttons.some(el => (el.innerText || el.textContent || '').trim() === '继续提交');
          const success = href.includes('/product/success.htm') && href.includes('primaryId=' + expected) && (href.includes('isSuccess=true') || text.includes('成功'));
          const logisticsError = text.includes('错误项物流提供方式');
          const pageTimeout = text.includes('timeout of 50000ms exceeded') || text.includes('504 Gateway') || text.includes('Gateway Time-out');
          return {{
            ok: success || hasContinue || logisticsError || pageTimeout,
            success,
            hasContinue,
            logisticsError,
            pageTimeout,
        ...
        if (submitState.success) {{
          finalResult = {{ status: 'success', message: 'submit success page confirmed', url: submitState.href }};
        }} else if (submitState.logisticsError) {{
          const logisticsRetry = waitFor(`() => {{
            const section = [...document.querySelectorAll('.sell-card, div, section')]
              .filter(el => (el.innerText || '').includes('物流提供方式') && (el.innerText || '').includes('智能运费模板'))
              .sort((a, b) => (a.innerText || '').length - (b.innerText || '').length)[0];
            if (section) section.scrollIntoView({{ block: 'center' }});
            const card = [...document.querySelectorAll('.LogisticsDeliveryMethod-container-content-list-office, [class*="LogisticsDeliveryMethod"][class*="office"], [class*="DeliveryMethod"], div, label')]
              .filter(el => (el.innerText || '').includes('智能运费模板') && ((el.innerText || '').includes('可计算核心国家') || (el.className || '').toString().includes('LogisticsDeliveryMethod')))
              .sort((a, b) => (a.innerText || '').length - (b.innerText || '').length)[0];
            if (!card) return {{ ok: false, message: 'smart freight template card not found on retry' }};
            card.scrollIntoView({{ block: 'center', inline: 'center' }});
            const clickable = card.querySelector('.LogisticsDeliveryMethod-select, input[type="radio"], input[type="checkbox"], .next-radio, .next-checkbox') || card;
            const r = clickable.getBoundingClientRect();
            const clickX = Math.max(8, r.left + Math.min(Math.max(r.width / 2, 18), 120));
            const clickY = r.top + r.height / 2;
            const target = document.elementFromPoint(clickX, clickY) || clickable;
            for (const type of ['pointerdown', 'mousedown', 'pointerup', 'mouseup', 'click']) {{
              target.dispatchEvent(new MouseEvent(type, {{ bubbles: true, cancelable: true, view: window, clientX: clickX, clientY: clickY }}));
            }}
            if (clickable !== target && typeof clickable.click === 'function') clickable.click();
            if (typeof card.click === 'function') card.click();
            const buttons = [...document.querySelectorAll('button')].filter(el => (el.offsetWidth || el.offsetHeight || el.getClientRects().length));
            const btn = buttons.reverse().find(el => (el.innerText || el.textContent || '').trim() === '提交');
            if (!btn) return {{ ok: false, message: 'submit button not found on logistics retry' }};
            btn.scrollIntoView({{ block: 'center' }});
            btn.click();
            return {{ ok: true }};
          }}`, 20000);
          if (!logisticsRetry.ok) throw new Error('logistics provider submit error: ' + logisticsRetry.message);
          sleep(3000);
          const retryState = waitFor(`() => {{
            const text = document.body ? document.body.innerText : '';
            const href = location.href;
            const expected = {json.dumps(expected_item_id)};
            const buttons = [...document.querySelectorAll('button')].filter(el => (el.offsetWidth || el.offsetHeight || el.getClientRects().length));
            ...
            const success = href.includes('/product/success.htm') && href.includes('primaryId=' + expected) && (href.includes('isSuccess=true') || text.includes('成功'));
            const logisticsError = text.includes('错误项物流提供方式');
            return {{ ok: success || hasContinue || logisticsError, success, hasContinue, logisticsError, href, text: text.slice(0, 700) }};
          }}`, 90000);
          if (retryState.success) {{
            finalResult = {{ status: 'success', message: 'submit success page confirmed after logistics retry', url: retryState.href }};
          }} else if (retryState.logisticsError) {{
            throw new Error('logistics provider submit error');
          }} else if (retryState.hasContinue) {{
            ...
            if (warningHandled.clicked) sleep(3000);
            const done = waitFor(`() => {{
              const text = document.body ? document.body.innerText : '';
              const href = location.href;
              const expected = {json.dumps(expected_item_id)};
              const success = href.includes('/product/success.htm') && href.includes('primaryId=' + expected) && (href.includes('isSuccess=true') || text.includes('成功'));
              return {{ ok: success, href, text: text.slice(0, 700) }};
            }}`, 90000);
            finalResult = {{ status: 'success', message: 'submit success page confirmed after logistics retry warning', url: done.href }};
          }}
        }} else if (submitState.pageTimeout) {{
          throw new Error('Alibaba page timeout after submit');
        }} else if (submitState.hasContinue) {{
          const warningHandled = JSON.parse(run(`JSON.stringify((() => {{
          const buttons = [...document.querySelectorAll('button')].filter(el => (el.offsetWidth || el.offsetHeight || el.getClientRects().length));
          const btn = buttons.reverse().find(el => (el.innerText || el.textContent || '').trim() === '继续提交');
          if (btn) {{
          ...
          const done = waitFor(`() => {{
            ...
            const pageTimeout = text.includes('timeout of 50000ms exceeded') || text.includes('504 Gateway') || text.includes('Gateway Time-out');
            return {{
              ok: (href.includes('/product/success.htm') && href.includes('primaryId=' + expected) && (href.includes('isSuccess=true') || text.includes('成功'))) || pageTimeout,
              success: href.includes('/product/success.htm') && href.includes('primaryId=' + expected) && (href.includes('isSuccess=true') || text.includes('成功')),
              pageTimeout,
              href,
              text: text.slice(0, 500)
            }};
          }}`, 120000);
          if (!done.success) throw new Error('Alibaba page timeout after continue submit');
          finalResult = {{ status: 'success', message: 'submit success page confirmed', url: done.href }};
        }} else {{
          throw new Error('unknown submit state: ' + JSON.stringify(submitState));
        }}
      }} else if (!finalResult) {{
...
def run(args: argparse.Namespace) -> None:
    ...
    for index, item_id in enumerate(ids, start=1):
        ...
        for attempt in range(1, args.attempts + 1):
            if attempt > 1:
                print(f"{index}/{len(ids)} [retry {attempt}/{args.attempts}] {item_id}", flush=True)
                if args.retry_sleep:
                    time.sleep(args.retry_sleep)
            result = chrome_process_product(url, args.publish, args.product_timeout)
            status = result.get("status", "failed")
            message = result.get("message", "")
            if status != "failed" or attempt >= args.attempts or not is_recoverable_error(message):
                break
        status = result.get("status", "failed")
        message = result.get("message", "")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--attempts", type=int, default=1, help="Attempts per product. Use 2 for slower but more reliable runs.")
    parser.add_argument("--retry-sleep", type=float, default=3.0, help="Seconds to pause before retrying a recoverable product failure.")
